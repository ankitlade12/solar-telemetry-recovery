...
def training_table(features, truth, contract, horizon, train_end, daylight_only=False):
    ends = target_ends(features, horizon)
    ...
    if daylight_only:
        selected &= geometry(ends, contract) > 0
    return features.iloc[np.flatnonzero(selected)], y[selected]


class QuantileForecaster:
    def __init__(self, profile="compact", seed=20260911, daylight_only=False):
    ...
    def fit(self, features, truth, contract, train_end, horizons=(1, 2, 3, 4)):
        ...
        for horizon in horizons:
            eligible, y = training_table(features, truth, contract, horizon, train_end, self.daylight_only)
            ...
            fitted = np.column_stack([m.predict(X) for m in self.models[horizon]])
            self.counts[horizon]["quantile_prediction_std"] = fitted.std(axis=0).tolist()
            self.counts[horizon]["training_crossing_fraction"] = float((np.diff(fitted, axis=1) < 0).any(axis=1).mean())
            self.counts[horizon]["training_target_zero_fraction"] = float((y == 0).mean())
        return self

    def predict(self, features):
        capacity = self.contract["capacity_kw_dc"]
        result = {h: np.maximum(0., np.sort(np.column_stack([m.predict(model_features(features, self.contract, h))
                    for m in models]), axis=1))*capacity for h, models in self.models.items()}
        if self.daylight_only:
            for h, q in result.items():
                q[geometry(target_ends(features, h), self.contract) <= 0] = 0.
        return result
    ...
    def transform(self, features, sample=False, seed=20260911):
        ...
        for lag in LAGS:
            ...
            for kind, mask, column in (("pv", pv_missing & ~ir_missing, 0),
                ...
                if sample:
                    # Counter-based draws keyed only by this forecast, slot,
                    # missingness type and round. Adding/reordering future rows
                    # must never change a previously issued stochastic forecast.
                    salt = {"pv": 1, "irradiance": 2, "joint": 3}[kind]
                    counter = features.index.asi8[rows].astype(np.uint64)
                    counter = counter ^ np.uint64((int(seed)+lag*0x9E3779B97F4A7C15+salt*0xD1B54A32D192ED03) % (1 << 64))
                    counter = (counter ^ (counter >> np.uint64(30)))*np.uint64(0xBF58476D1CE4E5B9)
                    counter = (counter ^ (counter >> np.uint64(27)))*np.uint64(0x94D049BB133111EB)
                    counter ^= counter >> np.uint64(31)
                    choice = (counter % np.uint64(self.k)).astype(int)
                    completed = self.pairs[neighbors[np.arange(len(rows)), choice]]
                else:
...
class ImputedForecaster:
    ...
    """
    def __init__(self, profile="compact", seed=20260911, k=50, rounds=10, daylight_only=False):
    ...
    def fit(self, features, truth, contract, train_end, horizons=(1, 2, 3, 4)):
        ...
        for horizon in horizons:
            eligible, y = training_table(completed, truth, contract, horizon, train_end, self.daylight_only)
            X = model_features(eligible, contract, horizon)
    ...
    def predict(self, features):
        ...
        for h in self.models:
            ...
            if self.daylight_only:
                night = geometry(target_ends(features, h), self.contract) <= 0
                for method in result:
                    result[method][h][night] = 0.
        return result
...
class DressedPersistence:
    ...
    """
    def __init__(self, geometric=False, daylight_only=False):
    ...
    def fit(self, features, truth, contract, train_end, horizons=(1, 2, 3, 4)):
        self.contract = contract
        for h in horizons:
            eligible, y = training_table(features, truth, contract, h, train_end, self.daylight_only)
    ...
    def predict(self, features):
        result = {h: np.maximum(0., self.point(features, h)[:, None]+values[self.strata(features, h)])
