    ...
    for site in config["sites"]:
        ...
        with threadpool_limits(limits=1):
            for profile in config["profiles"]:
                models[f"quantile_{profile}"] = QuantileForecaster(profile, config["seed"]).fit(
                ...
                models[f"imputed_{profile}"] = ImputedForecaster(profile, config["seed"], config["imputation_neighbors"],
                    config["imputation_rounds"]).fit(train_features, truth, contract, config["train_end"])
            ...
            for geometric in (False, True):
                ...
                models[name] = DressedPersistence(geometric).fit(train_features, truth, contract, config["train_end"])
