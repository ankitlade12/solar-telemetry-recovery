...
class SignedCalibrator:
    ...
    def issue(self, forecast_id, origin, horizon, base, features, target_daylight, target_end=None):
        ...
        # Explicit bin ends support HH:01 issue times without shifting truth bins.
        target_end = origin+horizon*HOUR if target_end is None else utc(target_end)
        if target_end <= origin:
            raise ValueError("Target interval must end after the forecast is issued")
        signed_adjustment(base, np.zeros(3))
        ...
        if eligible:
            record = IssuedForecast(forecast_id, origin, target_end, np.int64(horizon).item(),
