...
class ContextCalibrator(SignedCalibrator):
    def __init__(self, conditioning="physical", **kwargs):
        if conditioning not in {"physical", "physical_availability", "physical_recovery", "mask50"}:
    ...
    def correction(self, origin, horizon, features):
        ...
        saved = [f.mask if self.conditioning == "mask50" else f.state for f, _ in pool]
        if self.conditioning == "physical_availability":
            # Drop only the two recovery-age coordinates; preserve identical
            # kernel, recency, local/global mixing and availability bins.
            key = tuple(key[i] for i in (0, 2, 3, 5))
            saved = [tuple(value[i] for i in (0, 2, 3, 5)) for value in saved]
        match = np.array([value == key for value in saved])
        local_used = self.conditioning in {"mask50", "physical_recovery", "physical_availability"}
