"""Implement the frozen fixed-site estimands, paired blocks and figure tables."""
import argparse
import hashlib
import json
...
def plot_figures(output, tables):
    ...
    for ax, site in zip(axes, ("nist", "colorado")):
        ...
        ax.set(xticks=[0, 1], xticklabels=["0–6 h", "6–24 h"], title=site.upper() if site == "nist" else "Colorado",
    ...
    for ax, site in zip(axes, ("nist", "colorado")):
        ...
        ax.set(title=site.upper() if site == "nist" else "Colorado", xticks=[1, 2, 3, 4],
    ...
    fig, axes = plt.subplots(2, 2, figsize=(7.0, 4.2), constrained_layout=True, sharex=True)
    for j, site in enumerate(("nist", "colorado")):
        ...
        axes[0, j].set(title=site.upper() if site == "nist" else "Colorado", ylabel="NWIS")
...
def analyze(root, output=None):
    ...
    note.title = "Readme"
    for row in [
        ["Run", str(root)], ["Protocol", "research/FINAL_EVALUATION_PROTOCOL.md"],
        ["Evidence", "2017 fixed-site evaluation, 2016 primary training; recorded hourly AC targets"],
