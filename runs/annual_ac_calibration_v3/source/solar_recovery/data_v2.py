...
def normalize(raw, specification, capacity):
    raw = raw.copy()
    ...
    if raw.utc_measured_on.notna().all():
        ...
        times = expected_utc
        timestamp_source = "derived UTC: native timestamps plus seven hours; numeric catalog offset and uninterrupted local minute grid"
    else:
        raise ValueError("Mixed/missing UTC timestamps outside the explicit Colorado contract")
    raw["timestamp"] = pd.to_datetime(times, utc=True)
    ...
    invalid |= pv_mask & ((raw.value < -.05*capacity) | (raw.value > 1.5*capacity))
    invalid |= ~pv_mask & ((raw.value < -20.) | (raw.value > 2000.))
